...
use \Closure;
use Phalcon\NestedSets\NestedSets as Node;
use utilphp\util;
...
class SetMapper
{
    ...
    protected function mapTreeRecursive(array $tree, $parentKey = null, &$affectedKeys = array())
    {
        ...
        foreach ($tree as $attributes) {
            ...
            if ($parentKey) {
                $data[$node->getRootName()] = $parentKey;
                $result = $node->prependTo($node->findFirst("id = '$parentKey'"), $data);
            }else {
                $result = $node->makeRoot($data);
            }
